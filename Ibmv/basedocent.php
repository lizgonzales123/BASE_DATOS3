...
<body>
    ...
    <table class="table table-striped">
        ...
        <tbody>
            ...
            if (!empty($Nombre) && !empty($Apellip ) && !empty($ApellidoM) && !empty($Asignatura) && !empty( $Coreo )&& !empty( $Telefono)) {
                $instruccion_SQL = "INSERT INTO registrodocentes (Nombre, Apellidop, Apellidom, Asignatura, correo, Telefono) VALUES ('$Nombre', '$Apellip', '$ApellidoM', '$Asignatura', '$Coreo  ', '$Telefono')";
            ...
            if ($result->num_rows > 0) {
                while ($colum = $result->fetch_assoc()) {
                    ...
                    echo "<td>
                              <a href='editardocentes.php?id=" . $colum['id'] . "' class='btn btn-primary btn-sm'>
                                  <i class='fas fa-edit'></i> Editar
                              </a>
                              <a href='eliminardocentes.php?id=" . $colum['id'] . "' class='btn btn-danger btn-sm' onclick='return confirm(\"¿Estás seguro de que deseas eliminar este usuario?\")'>
    ...
    <a href="docentes.html" class="btn btn-success">Registrar Nuevo Docente</a>
