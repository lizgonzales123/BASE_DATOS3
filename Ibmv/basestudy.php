...
<body>
    ...
    <table class="table table-striped">
        ...
        <tbody>
            ...
            $Asignatura = isset($_POST['Carrera']) ? $_POST['Carrera'] : '';
            $Coreo = isset($_POST['correo']) ? $_POST['correo'] : '';
            $Telefono = isset($_POST['Nfecha']) ? $_POST['Nfecha'] : '';

            // Insertamos datos de registro al MySQL, indicando nombre de la tabla y sus atributos
            if (!empty($Nombre) && !empty($Apellip ) && !empty($ApellidoM) && !empty($Asignatura) && !empty( $Coreo )&& !empty( $Telefono)) {
                $instruccion_SQL = "INSERT INTO registrostudy (Nombre, Apellidop, Apellidom, Carrera, correo, Nfecha) VALUES ('$Nombre', '$Apellip', '$ApellidoM', '$Asignatura', '$Coreo  ', '$Telefono')";
            ...
            if ($result->num_rows > 0) {
                while ($colum = $result->fetch_assoc()) {
                    ...
                    echo "<td>
                              <a href='editarestudiantes.php?id=" . $colum['id'] . "' class='btn btn-primary btn-sm'>
                                  <i class='fas fa-edit'></i> Editar
                              </a>
                              <a href='eliminarestudiantes.php?id=" . $colum['id'] . "' class='btn btn-danger btn-sm' onclick='return confirm(\"¿Estás seguro de que deseas eliminar este usuario?\")'>
    ...
    <a href="estudiantes.html" class="btn btn-success">Registrar Nueva Carrera</a>
