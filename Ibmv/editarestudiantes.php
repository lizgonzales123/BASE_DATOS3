...
if (isset($_GET['id']) && is_numeric($_GET['id'])) {
    ...
    $sql = "SELECT * FROM registrostudy WHERE id = $id";  // Corregido aquí
...
if (isset($_POST['editar'])) {
    ...
    $Asignatura= $_POST['Carrera'];
    $correo= $_POST['correo'];
    $fecha= $_POST['Nfecha'];

    $sql = "UPDATE registrostudy SET Nombre =' $Nombre', Apellidop='$Apellidop', Apellidom='$Apellidom',  Carrera ='$Asignatura',  correo ='$correo',  Nfecha ='$fecha' WHERE id=$id";

    if ($conn->query($sql) === TRUE) {
        echo "Usuario actualizado correctamente";
        header("Location: basestudy.php"); // Redirige a la lista de usuarios
    } else {
...
<body>
    ...
    <div class="container">
        <form method="POST" action="">
            ...
            <input type="text" id="Carrera" name="Carrera" value="<?php echo $row['Carrera']; ?>" required><br><br>

            <label for="correo">Correo</label>
            <input type="email" id="correo" name="correo" value="<?php echo $row['correo']; ?>" required><br><br>

            <label for="Nfecha">fecha Nac</label>
            <input type="date" id="Nfecha" name="Nfecha" value="<?php echo $row['Nfecha']; ?>" required><br><br>
